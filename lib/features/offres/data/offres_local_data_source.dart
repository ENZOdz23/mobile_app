// lib/features/offres/data/offres_local_data_source.dart

import '../../../core/database/db_helper.dart';
import '../models/offer_model.dart';

class OffresLocalDataSource {
  final DBHelper _dbHelper;
  static const String _tableName = 'offres';

  OffresLocalDataSource({DBHelper? dbHelper})
    : _dbHelper = dbHelper ?? DBHelper();

  Future<List<OfferModel>> fetchOffers() async {
    try {
      final maps = await _dbHelper.query(_tableName);
      return maps.map((map) => OfferModel.fromMap(map)).toList();
    } catch (e) {
  ...
  Future<void> addOffer(OfferModel offer) async {
    try {
      await _dbHelper.insert(_tableName, offer.toMap());
    } catch (e) {
  ...
  Future<void> seedInitialData() async {
    final existingOffers = await fetchOffers();
    if (existingOffers.isEmpty) {
      final initialOffers = [
        OfferModel(
          ...
          shortDescription:
              'Connexion ultra-rapide et fiable pour votre entreprise',
          fullDescription:
        ...
        OfferModel(
          ...
          shortDescription:
              'Forfaits mobiles adaptés à vos équipes sur terrain',
          fullDescription:
